...
\begin{itembox}[l]{基本的な関数の導関数}
    \begin{eqnarray*}
        ...
        \dfrac{d}{dx}\mathrm{Sin}^{-1}\left(x\right)&=&\dfrac{1}{\sqrt{1-x^2}}\\
        \dfrac{d}{dx}\mathrm{Cos}^{-1}\left(x\right)&=&-\dfrac{1}{\sqrt{1-x^2}}\\
        \dfrac{d}{dx}\mathrm{Tan}^{-1}\left(x\right)&=&\dfrac{1}{1+x^2}\\
    \end{eqnarray*}
\end{itembox}
...
\begin{itembox}[l]{point}
    \begin{center}
        ...
        (\textgt{実際のひずみ} $\varepsilon'$) = (\textgt{応力(外力)によるひずみ} $\varepsilon$ ) + (\textgt{温度上昇によるひずみ} $\alpha \Delta T$)
    \end{center}
\end{itembox}
\subsection{BMDとSFD}
...
定常応答を求めるには,
\begin{eqnarray*}
    m \ddot{x}\left(t\right)+c\dot{x}\left(t\right)+kx\left(t\right)=F\cos\left(t\right)
\end{eqnarray*}
...
\begin{itembox}[l]{位相遅れ$\varphi$}
    \begin{eqnarray*}
        \varphi=\tan^{-1}\left(\dfrac{x_0\sin\varphi}{x_0\cos\varphi}\right)=\tan^{-1}\left(\dfrac{c\omega}{-m\omega^2+k}\right)
    \end{eqnarray*}
\end{itembox}
\subsubsection{複素振幅表示による方法}
\begin{eqnarray*}
...
\begin{center}
    \begin{tabular}{|c|p{50mm}|p{50mm}|}
        \hline
        \textgt{ミスパターン}                & \multicolumn{1}{|c|}{\textgt{原因}}                & \multicolumn{1}{|c|}{}                             \\ \hline
        式を写し間違える                     & 書き写した後に確認せず、そのまま計算を進めてしまう & 書き写した後、もう一度間違っていないか毎回確認する \\ \hline
        式を移項するときに符号をつけ間違える &                                                    &                                                    \\ \hline
        暗算のミス                           &                                                    &                                                    \\ \hline
    \end{tabular}
\end{center}
\end{document}