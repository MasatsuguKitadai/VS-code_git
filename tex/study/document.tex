...
これを\textgt{表面張力}といい、単位は$\left[\mathrm{\frac{N}{m}}\right]$である。
\begin{itembox}[l]{point}
    \begin{center}
        上下方向の力のつり合いを考える\\
        表面張力は,\textgt{境界面の接線方向}にはたらく\\
        ( \textgt{液体の重量$\left[\mathrm{N}\right]$} ) = ( \textgt{表面張力の垂直方向成分$\left[\mathrm{\frac{N}{m}}\right]$}) $\times$ ( \textgt{境界面の周の長さ}$\;\left[\mathrm{m}\right]$ )
    \end{center}
\end{itembox}
\subsection{粘性流体の力学}
